...
export default async function DashBoardPage() {
  ...
  return (
    <div className="p-4">
      ...
      <table className="tavle-auto w-full border-collapse border">
        <thead>
        ...
        <tbody>
          {posts.map((post) => (
            <tr key={post.id}>
              <td className="bourder p-2">{post.title}</td>
              <td className="bourder p-2 text-center">
                {post.published ? "表示" : "非表示"}
              </td>
              <td className="bourder p-2 text-center">
                {new Date(post.updatedAt).toLocaleString()}
              </td>
              <td className="bourder p-2 text-cernter">
